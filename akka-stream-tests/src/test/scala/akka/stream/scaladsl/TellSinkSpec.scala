...
class TellSinkSpec extends AkkaSpec {
  import TellSinkSpec._
  implicit val mat = ActorFlowMaterializer()

  "A TellSink" must {

    "send the elements to the ActorRef" in {
      Source(List(1, 2, 3)).runWith(Sink.tell(testActor, onCompleteMessage = "done"))
      expectMsg(1)
      expectMsg(2)
      expectMsg(3)
      expectMsg("done")
    ...
    "cancel stream when actor terminates" in {
      ...
      Source(publisher).runWith(Sink.tell(fw, onCompleteMessage = "done"))
